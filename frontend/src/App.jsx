...
export default function App() {
  ...
  // 🔥 YENİ: TRACKER LİSTESİ İÇİN FİLTRE VE AKORDİYON STATE'LERİ
        ...
        <div className={`${session?.role === 'ADMIN' || session?.role === 'TRACKER' ? 'w-full' : 'max-w-5xl'} mx-auto px-6 h-16 flex items-center justify-between transition-all duration-300`}>
          <div className="flex items-center space-x-3 cursor-pointer" onClick={() => setStep('INPUT')}>
            ...
            <div className="flex items-baseline space-x-2">
              ...
              <span className="text-[11px] font-mono uppercase tracking-widest text-neutral-400 font-medium hidden sm:inline">Protocol 18.16 (Stable Inline Setup)</span>
            </div>
          </div>
                ...
                <div className="absolute top-0 right-0 w-[70vw] sm:w-[240px] md:w-[260px] min-w-[200px] max-w-[290px] h-full bg-white shadow-[-10px_0_30px_rgba(0,0,0,0.1)] z-[400] flex flex-col border-l border-neutral-200 animate-in slide-in-from-right duration-300">
                  <div className="p-2.5 border-b border-neutral-100 bg-neutral-50/50 flex flex-col space-y-2">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      {/* 🔥 YENİ EKLENEN "BANA UYGUN TALEPLERİ GÖSTER" FİLTRESİ */}
                      {providerProfile && (
                  ...
                  <div className="flex-1 overflow-y-auto p-2 space-y-2 bg-neutral-50 pb-20">
                    ...
                    {filteredTrackerRequests.map(req => {
                      ...
                      // Akordiyonun açık olup olmadığı durumu
                      const isExpanded = expandedTrackerReqId === req.id;
                      
                      // Sağlayıcı zaten sırada mı?
                      const hasJoined = safeArray(req.queuedProviders).some(qp => Number(qp.id) === Number(providerProfile?.id)) || 
                                        ...
                                        providerRequests.some(pr => Number(pr.id) === Number(req.id));

                      // Anahtar kelime ile uyumlu mu?
                      ...
                      return (
                        <div 
                           key={req.id} 
                           onClick={() => { 
                             // Tıklanınca haritayı oraya götür
                             if(coords) {
                        ...
                        >
                          <div className="flex items-start justify-between mb-1">
                            ...
                            {/* Sırada rozeti en üstte görünsün */}
                          ...
                          {/* 🔥 AKORDİYON İÇİ İŞLEMLER (COLLAPSIBLE) */}
                          {isExpanded && (
