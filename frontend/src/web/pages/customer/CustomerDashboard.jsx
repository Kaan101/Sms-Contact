...
export default function CustomerDashboard() {
  ...
  const handleDeleteRequest = async (requestId) => { if (!window.confirm('Bu talebi iptal etmek istediğinize emin misiniz?')) return; try { await axios.post(`${API_BASE}/requests/${Number(requestId)}/status`, { newStatus: 'CANCELLED' }); await fetchCustomerData(); } catch {} };
                 ...
                 {activeCustomerRequests.map((req) => {
                    ...
                    return (
                      <div key={req.id} className="bg-[#FAFBFD] rounded-xl border border-neutral-200/90 p-4 shadow-sm space-y-3">
                        ...
                        {/* MÜŞTERİ KUYRUK VE AKSİYON ALANI (TAM SÜRÜM) */}
                                    ...
                                    return (
                                      <div key={qProv.id} className={`p-2.5 rounded-lg border text-xs flex items-center justify-between transition ${isCurrent ? (isSkippedByThis ? 'bg-rose-50 border-rose-200 shadow-sm' : 'bg-emerald-50 border-emerald-200 shadow-sm') : 'bg-white border-neutral-200'}`}>
                                        ...
                                        <div className="flex items-center space-x-2">
                                            {isCurrent && !isSkippedByThis && req.status === 'MATCHED' && (<button onClick={(e) => { e.stopPropagation(); handleStatusChange(req.id, 'ACCEPTED'); }} className="px-3 py-1.5 bg-emerald-600 text-white rounded text-[10px] font-bold"><ShieldCheck size={10} /><span>Onayla</span></button>)}
