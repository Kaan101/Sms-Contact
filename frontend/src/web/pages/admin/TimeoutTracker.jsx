...
export default function TimeoutTracker() {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case 'WAITING':
        return <span className="px-2 py-1 bg-amber-100 text-amber-800 rounded text-[10px] font-bold tracking-wider flex items-center space-x-1 w-max"><Clock size={10} /><span>BEKLİYOR</span></span>;
      case 'MATCHED':
        return <span className="px-2 py-1 bg-emerald-100 text-emerald-800 rounded text-[10px] font-bold tracking-wider flex items-center space-x-1 w-max"><CheckCircle size={10} /><span>EŞLEŞTİ</span></span>;
      case 'TIMEOUT':
        return <span className="px-2 py-1 bg-rose-100 text-rose-800 rounded text-[10px] font-bold tracking-wider flex items-center space-x-1 w-max"><AlertCircle size={10} /><span>ZAMAN AŞIMI</span></span>;
      default:
        return null;
    }
  };
  ...
  const getTimeoutStyle = (status) => {
    if (status === 'TIMEOUT') return 'text-rose-600 font-bold bg-rose-50 px-2 py-0.5 rounded border border-rose-100 w-max';
    if (status === 'MATCHED') return 'text-neutral-400 line-through w-max';
    return 'text-amber-600 font-semibold animate-pulse w-max';
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-5 rounded-2xl border border-neutral-200 shadow-sm">
        <div>
          <h2 className="text-xl font-extrabold text-neutral-900 tracking-tight flex items-center gap-2">
            <ShieldAlert className="text-neutral-950" size={24} /> Timeout ve Talep İzleme
          </h2>
          <p className="text-sm text-neutral-500 font-medium mt-1">Sistemdeki tüm eşleşmeleri ve zaman aşımlarını canlı takip edin.</p>
        </div>
        <button className="flex items-center space-x-2 px-4 py-2 bg-neutral-50 border border-neutral-200 text-neutral-700 rounded-xl text-sm font-bold shadow-sm hover:bg-neutral-100 transition cursor-pointer">
          <RefreshCw size={16} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-neutral-200 shadow-sm flex items-center space-x-4">
          ...
          <div><p className="text-xs font-bold text-neutral-500 uppercase tracking-wider">Toplam</p><h3 className="text-2xl font-black text-neutral-900">{stats.total}</h3></div>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-neutral-200 shadow-sm flex items-center space-x-4">
        ...
        <div className="p-4 border-b border-neutral-200 flex flex-col sm:flex-row justify-between items-center gap-4 bg-neutral-50/50">
          <div className="flex space-x-2 w-full sm:w-auto overflow-x-auto pb-2 sm:pb-0">
            {['ALL', 'WAITING', 'MATCHED', 'TIMEOUT'].map(f => (
              ...
              >
                {f === 'ALL' ? 'Tümü' : f === 'WAITING' ? 'Bekleyenler' : f === 'MATCHED' ? 'Eşleşenler' : 'Zaman Aşımı'}
              </button>
            ))}
          </div>
            ...
            <tbody className="text-sm font-medium text-neutral-700 divide-y divide-neutral-100">
              {filteredDemands.map((demand) => (
                <tr key={demand.id} className="hover:bg-neutral-50/80 transition">
                  ...
                  <td className="p-4">
                    {demand.provider !== '-' ? (
                      <span className="inline-flex items-center space-x-1.5 text-xs font-bold text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-md border border-emerald-100 w-max">
                        <MapPin size={10} />
